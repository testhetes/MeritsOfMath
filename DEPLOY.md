...
|---|---|---|---|
| `GROQ_API_KEY` (env var) | Groq | `qwen/qwen3.6-27b` | fast primary. Qwen3 is a reasoning model; the proxy sends `reasoning_effort: 'none'` for it, otherwise it spends the small token budget thinking and returns nothing |
...
| `OPENROUTER_API_KEY` (env var) | OpenRouter | `meta-llama/llama-3.3-70b-instruct:free` | shared free pool, often rate-limited; best-effort last layer |
| `GEMINI_API_KEY` (env var) | Google Gemini | `gemini-2.0-flash` | not in the default order; add it via `PROVIDER_ORDER` if its free tier works in your region |
...
such as `timeout` when retrieval fell back to an ungrounded reply).
...
5. **Settings → Bindings**: add **Workers AI** named `AI` (embeddings and the second provider)
   and the **Vectorize** index `merits-kb` named `VECTORIZE`.
...
- `/api/chat` only exists where Pages Functions run, and it needs the `AI` and `VECTORIZE`
  bindings. The simplest way to exercise it is against the deployed site. Running Functions
  locally with `npx wrangler pages dev .` would also need those bindings configured for local
  use, which this project has not set up.
- The Python test suite runs against a deployed site: see `tests/conftest.py` for the
  `RAG_BASE_URL` / `INGEST_SECRET` variables. Write tests stay skipped unless
  `RAG_ALLOW_PROD_WRITES=1` is set deliberately.
...
back to its cache only when the network fails. CDN libraries (pinned MathJax, marked, DOMPurify
and Google Fonts) are cached after the first successful load, and only when the download is
verifiably complete. Bump `CACHE` in `sw.js` only to purge everything, for example when files
are deleted — routine deploys don't need it.
