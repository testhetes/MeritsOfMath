...
| Contents today | 143 vectors from 24 Markdown lessons under `content/` (grades 1–5 and `nang-cao`) | — |
...
Running the chunker over all 24 lesson files (2026-09-17, after the four
Nâng cao · Nhóm nhân tử lessons were added) gives:

```
24 docs, 143 chunks total (matches the index's 143 vectors)
chunks per doc: min 3, mean 6.0, max 7
chunk length (chars): min 46, mean 179.8, median 164, max 560 (chunker's own cap is 1200 — never reached by real content)
```
...
- This index: 143 vectors × 1024 dims.
- `queries_made ≤ 30,000,000 / 1024 − 143 ≈ 29,297 − 143 ≈ 29,154` (the −143
  term is negligible at this scale).
- **≈29,150 grounded messages/month** (≈972/day averaged over 30 days) before
...
- Stored today: 143 × 1024 = 146,432 dimensions, against a 5,000,000 free
  budget → **2.9% used.**
- Remaining: (5,000,000 − 146,432) / 1024 ≈ **4,739 more vectors** could be
  stored free. At the measured 6.0 chunks/lesson average, that is roughly
  **≈790 more lessons** — storage is not a near-term constraint at all.
