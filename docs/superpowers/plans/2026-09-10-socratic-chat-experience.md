...
- **Deploy target:** `origin` = `testhetes/MeritsOfMath`, auto-deploys `main` to `https://meritsofmath.pages.dev`. Work on `feat/socratic-chat`, the branch this plan was actually executed on. Deploy by fast-forwarding `main` from it and pushing, because `INGEST_SECRET` and the `VECTORIZE` binding exist only in the **Production** environment. Earlier drafts named `feat/socratic-rag-tutor` and used plain merges; the deploy commands inside the completed Tasks 0–3 still do, and Tasks 4 and 5 carry the corrected commands.
...
- **Never add a root-level `conftest.py`.** `tests/conftest.py` declares `pytest_plugins = ["pytester"]`, which pytest rejects outright when a non-rootdir conftest carrying it is imported after configure. It is harmless today — 126 tests collect cleanly both from the repo root and from `tests/` — but a root `conftest.py` would turn it into a hard collection failure for the whole suite. This plan adds test files: put them in `tests/`.
- **Never let the author of content write the eval questions for it unchecked.** Plan 1 lost a full review cycle to exactly this: the same author wrote the 16 lessons and the questions testing retrieval of them, and unconsciously reused their own phrasing — producing an eval that measured keyword overlap while appearing to measure meaning. A mechanical 5-token overlap guard then caught two confounded cases that careful expert reading had missed. `tests/test_retrieval_eval.py` enforces the guard; any case added anywhere must pass it.
- **The primary LLM is `qwen/qwen3.6-27b` on Groq's free tier, and it must be sent `reasoning_effort: "none"`.** Groq retired `llama-3.3-70b-versatile` on 2026-08-16, and for weeks every chat request silently fell back to Workers AI without anything alerting. Qwen 3 reasons by default *inside* `message.content`, so without that parameter children would receive `<think>` monologues as an HTTP 200 — which the fallback chain never catches, because it only reacts to 429, 5xx and network errors. Task 0b made that change; do not undo or broaden it.
- **`tests/test_chat_providers.py` must stay green.** It forces each provider individually, so a retired model, a leaked reasoning block or an empty reply fails the same day instead of degrading unnoticed. If a later change makes it fail, that is a real regression — never skip or weaken it to get a green run.
- **Do not set a `GROQ_MODEL` environment variable in Cloudflare.** The default in `chat.js` is the single source of truth. A stale `GROQ_MODEL` still naming the retired model is exactly what kept Groq broken after the code fix had already deployed.
- **The user knowingly accepted free-tier capacity below the pre-flight gate:** roughly 150–210 grounded messages a day on Groq (its 200,000 tokens/day cap binds first), about 2.5–3.5 children chatting at once (8,000 tokens/minute), and about 250–360 a day once Workers AI absorbs overflow. This is a deliberate zero-cost trade, recorded in `docs/RAG-OPERATIONS.md`. Revisit it before real classroom use; do not treat it as a defect to fix inside this plan.
